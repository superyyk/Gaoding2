		...
		<view class="key_main">
			...
			<view class="main_content">
				<view class="content_num">
					<view v-for="(item,index) in inputArray" :key="index" style="font-size: x-large;font-weight: 500;" class="content_item">{{password[item-1] ? '●' :''}}</view>
				</view>
				<view class="main_forget" @tap="forgetFun">忘记密码</view>
			</view>
			<view class="main_keyboard">
				<view class="key_num" style="font-size: x-large;font-weight: 500;" v-for="(item,index) in numberArray" :key="index" @tap="inputNumFun({num:item})">{{item}}</view>
				<view class="key_null"></view>
				<view class="key_0" style="font-size: x-large;font-weight: 500;"  @tap="inputNumFun({num:0})">0</view>
				<view hover-class="hv" class="key_del" @tap="delNumFun">
