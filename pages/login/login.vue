<template>
	<view>
		...
		<view class="u-f-dc m-t-10">
			<view class="u-f-dr u-f-ac"  style="background-color: #f1f1ff; border-radius: 20px;">
				<view>
		...
		<view class="u-f-dc m-t-10">
			<view class="u-f-dr u-f-ac" style="background-color: #f1f1ff; border-radius: 20px;">
				<view>
					<image src="../../static/images/yanzhengma002.png" mode="aspectFit" style="width: 20px; height: 35px; margin-left: 12px;"></image>
				</view>
...
<!-- 		<view class="u-f-dr p-10 u-f-ac">
		...
		<!-- <view  style="width: 50%;  color: #FFFFFF; border-radius: 5px; margin-top: 15px; height: 45px;text-align: center; font-size: large; padding: 5px 30px; background-color:#f9710b;" >
			...
			<button type="default" @click="login()" style="background-color:#424554 ; color: #FFFFFF; width: 150px;">立即登陆</button>
...
<script>
	...
	uni.navigateTo({
		...
		let user_uuid=this.checkLogin().user_uuid
		...
		methods: {
			...
			login() {
				...
				if (!num.test(this.code)) {
					...
					console.log(_self.code_uid)
						uni.request({
						url:this.tp_url+'f_login_confirm',
						data: { 
							tel: this.phoneNum, 
							code: this.code,
			...
			async create_user(res){
				// let op={
				// 	url:this.tp_url+'f_login_confirm',
				// 	data:{
				// 		user_uuid:this.code_uid,
				// 		tel:this.phoneNum
				// 	}
				// }
